...
원본 영수증 50개 중 명시적으로 선택된 40개를 Worker 한 명이 검토하는 연구용 웹 애플리케이션입니다. 화면은 `Instructions → Start → Randomized 40 Tasks → Completion`만 제공하며 `/admin`이나 연구자용 대시보드는 없습니다. 연구 결과는 Supabase의 테이블과 View에서 확인합니다.
...
- 제공된 영어 이미지 50개와 `answer_key_en.json`의 50개 행은 그대로 보존하고 1:1로 검증합니다.
- 신규 세션은 `1, 19, 23, 27, 33, 35, 38, 41, 46, 47`을 제외한 40개만 Task/Scoring pool로 사용합니다.
...
1. 공통 제외 설정으로 원본 50개에서 40개 eligible ID를 먼저 만듭니다.
2. Node `crypto.randomInt()`를 사용하는 Fisher–Yates shuffle로 eligible 40개만 섞습니다.
3. DB 함수가 40개 eligible ID가 모두 존재하고 중복·제외 ID가 없는지 재검증합니다.
4. `receipt_order`, `started_at`, `status = started`를 한 번만 저장합니다.
...
- 20분을 넘겨도 응답 제출과 나머지 Task 진행은 막지 않습니다. 시간 초과 여부는 별도 연구 지표로 저장하며 Worker 분류는 완료 여부와 정확도로만 결정합니다.
...
- 완료 RPC는 응답 테이블을 다시 집계하고 해당 세션의 `total_tasks`와 일치할 때만 완료합니다. 신규 세션은 40개이며 기존 50개 세션도 계속 지원합니다.
...
4. 기존 프로젝트에는 이어서 [`supabase/migrations/004_accuracy_threshold_90.sql`](supabase/migrations/004_accuracy_threshold_90.sql)을 실행합니다.
5. 모든 프로젝트에서 마지막으로 [`supabase/migrations/005_receipt40_pool_and_classification.sql`](supabase/migrations/005_receipt40_pool_and_classification.sql)을 실행합니다. 신규 세션만 40문제 규칙을 사용하며 기존 50문제 행과 응답은 보존됩니다.
6. Project Settings → API Keys에서 Project URL과 `sb_secret_...` 형식의 server-side Secret key를 확인합니다.
7. 로컬 `.env.local` 또는 Vercel Environment Variables에 아래 값을 설정합니다.
...
- 진행: `current_index`, `current_shown_at`, `receipt_order`, `experiment_version`
...
- `inattentive_accuracy_threshold = 0.90` 한 곳에서 정확도 threshold를 관리합니다. 신규 40문항 세션은 36문항 이상 정답인 완료 Worker만 `NORMAL`입니다.
...
| `inattentive_workers` | 정확도 threshold 미만인 완료 Worker(연구용 operational classification) |
...
`INATTENTIVE`는 완료 Worker의 낮은 정확도를 바탕으로 한 연구용 분류일 뿐 실제 부주의를 단정하지 않습니다. 20분 초과는 별도 지표이며 분류를 바꾸지 않습니다.

- Completed worker Task Accuracy = `correct_tasks / total_tasks` (신규 세션은 40)
...
실제 API end-to-end 테스트에는 migration이 적용된 Supabase 테스트 프로젝트의 환경 변수가 필요합니다. 단위 테스트는 40개 pool과 제외 목록, 분류 경계값, 가격/텍스트 정규화, 과도한 fuzzy 거부, Fisher–Yates 보존성, 50개 원본 asset 매칭, Client/API 정답 비노출을 검사합니다.

Migration 적용 뒤 Supabase SQL Editor에서 [`supabase/tests/classification_scenarios.sql`](supabase/tests/classification_scenarios.sql)을 실행하면 36/40 Normal, 35/40 Inattentive, 40/40 Normal, 35문항 진행 후 Dropout, 제외 Receipt 미생성을 검증합니다. 전체가 transaction 안에서 실행되고 마지막에 rollback됩니다.
